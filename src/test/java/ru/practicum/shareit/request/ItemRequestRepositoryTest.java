...
class ItemRequestRepositoryTest {
    ...
    void findAllByRequesterNotLikeOrderByCreatedAsc() {
        ...
        itemRequestRepository.save(ItemRequest.builder().description("description").requester(user)
                .created(LocalDateTime.now()).build());
        assertThat(itemRequestRepository.findAllByRequesterNotLikeOrderByCreatedAsc(user, Pageable.ofSize(10))
                .stream().count(), equalTo(0L));
        User user2 = userRepository.save(User.builder().name("name2").email("email2@email.com").build());
        assertThat(itemRequestRepository.findAllByRequesterNotLikeOrderByCreatedAsc(user2, Pageable.ofSize(10))
